...
class GetCurrentPose_Response(metaclass=Metaclass_GetCurrentPose_Response):
    ...
    def __init__(self, **kwargs):
        ...
        self.euler_rpy = array.array('d', kwargs.get('euler_rpy', []))
    ...
    def __eq__(self, other):
        ...
        if self.pose != other.pose:
            return False
        if self.euler_rpy != other.euler_rpy:
            return False
        return True
    ...
    @builtins.property
    def euler_rpy(self):
        """Message field 'euler_rpy'."""
        return self._euler_rpy

    @euler_rpy.setter
    def euler_rpy(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'euler_rpy' array.array() must have the type code of 'd'"
            self._euler_rpy = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'euler_rpy' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._euler_rpy = array.array('d', value)
...
class Metaclass_GetCurrentPose(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'denso_motion_control.srv.GetCurrentPose')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from denso_motion_control.srv import _get_current_pose
...
class GetCurrentPose(metaclass=Metaclass_GetCurrentPose):
    from denso_motion_control.srv._get_current_pose import GetCurrentPose_Request as Request
    from denso_motion_control.srv._get_current_pose import GetCurrentPose_Response as Response
