...
def launch_setup(context, *args, **kwargs):
    # Resolve to a real string BEFORE xacro runs
    ...
    controllers_yaml = xacro.load_yaml(
        os.path.join(
            get_package_share_directory("staubli_tx40_moveit_config"),
            "config",
            "staubli_tx40_controllers.yaml",
        )
    )
    moveit_controllers = {
    ...
    rviz_base = os.path.join(get_package_share_directory("staubli_tx40_moveit_config"), "config")
    ...
    # NOTE: give the static TF its 6 numeric args, otherwise newer
    # tf2_ros static_transform_publisher warns / can misbehave
    static_tf_node = Node(
        package="tf2_ros",
